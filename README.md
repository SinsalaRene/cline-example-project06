...
> Noticed the difference in the -b and -ub values. Switched to way lower values and doubled the token gen speed.
> Seems pretty stable
> Not sure if or how much this reduces quality though.
> Just noticed that this is the older module...
```bash
./build/bin/llama-server \
...
> testing with unc model and same params -> Still slow 25 t/s Switch back to model and params above in between prompt 2
```bash
./build/bin/llama-server \
  -m ../LLM-Models/Qwen3.6-35B-A3B-Uncensored-HauhauCS-Aggressive-IQ3_M.gguf \
  --alias "qwen3.6-35b" \
